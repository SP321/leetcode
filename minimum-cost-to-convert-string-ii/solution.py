...
class Solution:
    def minimumCost(self, source: str, target: str, original: List[str], changed: List[str], cost: List[int]) -> int:
        ...
        for k in original_set&changed_set:
            for i in original_set:
                for j in changed_set:
                    g[i,j]=min(g[i,j],g[i,k]+g[k,j])

        n=len(source)
        m=len(target)

        original_search = AhoCorasick(list(original_set))
        result = original_search.search_words(source)
        source_candidates=defaultdict(list)
        for word, positions in result.items():
            for pos in positions:
                source_candidates[pos].append(word)

        changed_search = AhoCorasick(list(changed_set))
        target_candidates=defaultdict(list)
        result = changed_search.search_words(target)
        for word, positions in result.items():
            for pos in positions:
                target_candidates[pos].append(word)
        ...
        def dp(i):
            if i==n:
                return 0
            possible_changes = []
            x=set(source_candidates[i])
            y=set(target_candidates[i])
            for u in x:
                for v in y:
                    if g[u,v]!=float('inf'):
                        possible_changes.append((len(u), g[u, v]))
            ans=float('inf')
            if source[i]==target[i]:
                ans=dp(i+1)
            for size,c in possible_changes:
                ans=min(ans,dp(i+size)+c)
            return ans
        ans=dp(0)
